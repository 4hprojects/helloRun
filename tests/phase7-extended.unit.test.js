...
describe('Phase 7 Extended Features', () => {
  ...
  describe('Real-Time Check-In Service', () => {
  ...
  describe('Result Import Validation Service', () => {
    ...
    test('validateResultRow: should validate time format', () => {
      ...
      // Accept HH:MM:SS or MM:SS format (elapsed time can exceed 24 hours)
      // But minutes and seconds must be 00-59
    ...
    test('timeToMilliseconds: should convert time strings to ms', () => {
      const conversions = {
        '01:23:45': 1000 * (1 * 3600 + 23 * 60 + 45),  // 1h 23m 45s = 5025 seconds
        '05:30': 1000 * (5 * 60 + 30),                  // 5m 30s = 330 seconds
        '00:00:01': 1000                                // 1 second
      };

      Object.entries(conversions).forEach(([timeStr, expectedMs]) => {
        // Parse logic matching the service implementation
        ...
        assert.strictEqual(ms, expectedMs, `Conversion failed for ${timeStr}: expected ${expectedMs}, got ${ms}`);
      });
    });
