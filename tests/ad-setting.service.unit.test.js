...
test('AdSense bootstrap can load for Google CMP while manual placements remain independently empty', () => {
  const settings = normalizeAdSettings({
    enabled: 'on',
    scriptEnabled: 'on',
    group_blogPost_enabled: 'on'
  ...
  assert.equal(shouldLoadAdScript(settings, 'blogPost'), true);
  assert.equal(canRenderAdPlacement(settings, 'blogPost', 'in_article'), false);
});
