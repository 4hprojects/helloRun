...
test('initial migration manifest contains exactly the approved 24 indexed and 60 noindex guides', () => {
  assert.equal(INDEX_CANDIDATE_SLUGS.length, 24);
  assert.equal(NOINDEX_SLUGS.length, 60);
  assert.equal(new Set([...INDEX_CANDIDATE_SLUGS, ...NOINDEX_SLUGS]).size, 84);
