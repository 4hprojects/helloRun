...
test('initial migration manifest contains exactly the approved 22 indexed and 47 noindex guides', () => {
  assert.equal(INDEX_CANDIDATE_SLUGS.length, 22);
  assert.equal(NOINDEX_SLUGS.length, 47);
  assert.equal(new Set([...INDEX_CANDIDATE_SLUGS, ...NOINDEX_SLUGS]).size, 69);
