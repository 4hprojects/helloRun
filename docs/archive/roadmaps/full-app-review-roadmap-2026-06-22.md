# HelloRun — Full App Review & Roadmap (Historical)

> Archived July 31, 2026. This document preserves the former mixed audit,
> completion log, and roadmap. See [`../../ROADMAP.md`](../../ROADMAP.md) for
> current forward-looking priorities and
...
|---------|-----------|-------|
| Full codebase review — process/efficiency/intuitivity/security | Jul 6 | Docs only, no code changed. Second fresh-eyes pass, deliberately scoped to what the July 5 analysis did *not* cover (its P0–P2 fixes were re-verified in source and hold): session lifecycle, proxy/IP topology, deploy pipeline, error-surface UX. 23 findings in `docs/archive/reviews/2026-07-06-codebase-review/` — `README.md` (4-phase plan), `01-security.md` (SEC-1…9), `02-efficiency.md` (EFF-1…4), `03-process-dx.md` (PROC-1…6), `04-ux-intuitivity.md` (UX-1…4). **Headliners:** no `req.session.regenerate()` at any login entry point (session fixation, SEC-1); `req.session.user = user` writes the full user doc incl. `passwordHash` + reset tokens into the Mongo sessions collection and nothing ever reads it (SEC-2); production rate limiters run per-process in-memory because `REDIS_URL` is unset on Render (SEC-3); `trust proxy = 1` behind the two-hop Cloudflare→Render chain likely makes `req.ip` a Cloudflare edge IP — rate-limit buckets shared across strangers and spoofable via leftmost-XFF in `getRequestIpAddress` (SEC-4); `POST /login` is the only auth mutation without CSRF (SEC-5); no CI — pushes to main auto-deploy untested (PROC-1); nothing stops `npm run test:integration` from targeting production (PROC-2, the July 2 incident's failure mode); no SIGTERM handling though Render sends it every deploy (EFF-2); 12 plain-text 403/429 responses bypass the styled error page (UX-1). Verified-healthy sections in each doc record what was checked and clean (regex escaping, uploads, webhooks HMAC, reset-token atomicity, indexes, PG pool) so future passes don't re-audit |
| Analysis CQ-2 — split the three oversized files | Jul 6 | Split the three files the analysis flagged, each as a **pure verbatim move** (code sliced by acorn AST position and relocated byte-for-byte, never retyped) and each committed separately for independent revertability. **`page.controller.js` 3,564→14-line barrel:** eight concern sub-controllers under `src/controllers/page/` (`home`, `event`, `badge`, `registration`, `submission`, `blog-public`, `leaderboard`, `sitemap`) plus `_shared.js` holding all imports and cross-concern helpers; the mutable `syncRegistrationPaymentShadow` binding and its `__set/__reset` test hooks were kept together in `submission.controller.js` so reassignment still propagates, and one of two byte-identical `escapeXml` copies was dropped. **`blog.controller.js` 2,583→barrel:** `blog/{public,author,admin}.controller.js` + `_shared.js` (imports, the FeedCtor promise cache, ~40 shared helpers). **`submission.service.js` 2,111→1,739:** extracted the OCR-parsing / distance-plausibility / auto-approval-decision cluster — first proven to be a leaf (grep-confirmed it makes no calls back into the orchestrator) — into a new `submission-validation.service.js` that requires nothing, so no cycle is introduced (this service has prior circular-dependency history via accumulated-activity's lazy require); the orchestrator imports back the ten names it still uses. Deliberately conservative: the accumulated-challenge logic is entangled with the orchestrator's snapshot/eligibility functions and was left in place rather than risk a runtime cycle with no live DB to verify against. **Method/tooling:** built a reusable acorn-based scope-aware no-undef checker (the real safety oracle — `node --check` only catches syntax, not a helper reference left behind in the old file), plus an exports-snapshot oracle and a multiset line-diff for content preservation. Per-file verification: exports byte-identical to the pre-split module (page 35/35, blog 34/34, submission 20/20); no-undef clean on every generated file; content diff shows zero change beyond the intentional `escapeXml` dedupe; barrels + sub-modules + downstream dependents all `require()` cleanly; full DB-free unit suite 315/315 after repointing four source-scan tests (`audit-source`, `submission-idempotency-source`, `blog-admin-queue-source`) at the split files. CQ-3 (organiser auth-chain unification) intentionally NOT bundled — it changes auth semantics, not layout, and needs a running app to verify |
| Organizer onboarding simplification — implementation (all 4 phases) | Jul 6 | Implemented the full `docs/implementation/organizer-onboarding-simplification.md` proposal; the doc now carries an Implementation Tracker with per-phase priorities and start/completion timestamps (P1+design doc Jul 5→6, P2–P4 Jul 6). **Phase 1 — risk-based gating:** `User.canCreateEvents()` and `canCreateEventsFromLeanUser` (`auth.middleware.js`, now exported for tests) accept any verified `organiser` with a signed acknowledgement regardless of application status (plus a new restricted-account guard); the acknowledgement route/dashboard modal open to `not_applied`/`rejected` (copy rewritten around "unverified identity" and free-virtual capability); new `getRestrictedSetupReasons()` (exported from `event-approval.service.js` with the four `has*Setup` helpers) blocks paid fee mode, payment setup, physical rewards, delivery, and onsite logistics at `POST /create-event` and `POST /events/:id/edit` for non-approved organizers with a "verify to unlock" error (uploaded branding cleaned up on rejection); `/events*` management routes (list/details/edit/status/media/badges — 10 routes) moved `requireApprovedOrganizer`→`requireCanCreateEvents` so acknowledged organizers can manage what they create (`/promote*` unchanged); the application form is per-type — individuals skip business name/address/business-proof entirely (server defaults `businessName` to the account name) while company/NGO/sports-club now **require** registration number, business address, and business proof (client + server). **Phase 2 — UX:** application-status page "What You Can Do Right Now" panel (ack-aware CTA), verification form leads with capability framing, step 1 relabeled "Organizer Info", address maxlength mismatch fixed (200→500). **Phase 3 — graduated auto-publish:** `event_auto_approval_v2_trusted_organizer` in `getAutoApprovalEligibility` — organizer approved + emailVerified + `accountStatus` active + ≥3 completed events (published/closed, past `eventEndAt`, non-test, env-overridable threshold) unlocks paid/physical/delivery auto-publish; onsite/hybrid and readiness/organizer failures stay manual; rule version recorded via existing `autoApprovalRuleVersion`; completed-event count injectable for DB-free tests. **Phase 4 — OCR-assist:** new `id-ocr.service.js` runs tesseract.js **server-side** (self-hosted tessdata dir, lazy worker, 30s timeout, injectable recognizer) and scans the whole ID text for the account name with the client matcher's normalization + 0.75 per-part similarity (statuses `matched`/`not_detected`/`not_checked` — deliberately no `mismatched`, an ID scan can't distinguish "different person" from "unreadable"); verdict + detected name + duplicate-phone count persist on `OrganiserApplication` (survive the document purge), render as badges in admin application details, and `not_detected` auto-routes the application to the previously-never-set `under_review` status (already reviewable/filterable in admin, zero controller changes); OCR failure/PDF/timeouts degrade to `not_checked` and never block submission | Verification: 24 new DB-free tests across `organizer-gating` (8, incl. capability truth table against the real middleware export), `event-auto-approval-trusted` (6, real `getPublishReadinessErrors` fixtures), `id-ocr-name-match` (10, noisy PH-ID fixture text, timeout/PDF/error degradation); full unit suite **315/315**; all 7 touched EJS templates compile; live smoke pending (no staging): real signup→acknowledgement→free-event flow, real ID upload OCR verdict, Render deploy tessdata availability |
| Analysis P3 debt batch (partial: CQ-1, CQ-4, CQ-5-boot, multer) | Jul 6 | **CQ-4 dead code:** deleted `src/config/db.js` (a second, unused Mongo connector — `server.js` has its own) and `src/routes/event.routes.js` (unmounted, byte-identical to `certificateVerification.routes.js`); verified zero requires/mounts before deletion. **CQ-1 template sweep:** audited every `<%-` across all 131 EJS views (46 non-include occurrences). Fixed the unsafe ones: organizer-controlled `galleryUrl`/`posterImageUrl` emitted raw into `data-*` attributes on the **public** event-details page and organizer event pages, and `eventDetailsMarkdown` raw in a hidden-input value (attribute-breakout stored XSS, organizer→runner) — all switched to `<%=`; 15 unescaped `JSON.stringify` script bootstraps hardened with `.replace(/</g, '<')` (create/edit-event waiver+details+pricing, event-register pricing preview x2, run-proof modal initial data, shop-checkout summary, badge-verification JSON-LD, admin events-list/event-detail, privacy-policy editor). Confirmed-legit raw outputs kept: sanitized `contentHtml`/`policyHtml`/`eventDetailsHtml`/`contentHtmlPreview`, layout `body`, blog `safeJson`. **Biggest find — waiver XSS:** `renderWaiverTemplate()` (`utils/waiver.js`) did **no sanitization** and its output renders raw on the runner-facing registration page; since pending organizers can self-serve into live free events, any signup could serve `<script>` to every registering runner. Now sanitized via `utils/sanitize.js` (allowlist: div/h2-h4/p/br/strong/em/u/ul/ol/li/blockquote/a), applied *after* placeholder substitution so organizer-name injection is covered; verified hostile payload stripped and the default template survives byte-for-byte minus whitespace. Convention documented in CLAUDE.md Architecture Notes and **enforced**: `tests/blog-template-escaping.unit.test.js` now walks all of `src/views/` and fails on any `<%-` not matching the allowlist (include / hardened-JSON / safeJson / body / named sanitized fields). **CQ-5 (boot scope):** new `src/config/validate-env.js` — fatal on missing `SESSION_SECRET`/`MONGODB_URI`; warns on missing DATABASE_URL/APP_URL/RESEND_API_KEY/EMAIL_FROM/Turnstile/R2 vars, missing REDIS_URL (in-memory limiter note), and non-numeric duration vars (PASSWORD_RESET_EXPIRY etc.); wired into `server.js` replacing both inline checks; full `process.env` read-site centralisation deliberately not attempted. **Deps:** `multer` 1.4.5-lts.1 → 2.2.0; `npm audit` 0 vulns; `upload.service` loads and its `.single`/`.fields` usage is unchanged in 2.x (live upload smoke after deploy still worthwhile). Deferred: CQ-2 file splits and CQ-3 organiser-auth unification (organiser routes actively being edited by the onboarding workstream). Verification: repo-wide escaping test green; `tests/validate-env.unit.test.js` 5/5; 11 templates compile; full DB-free suite 299/299. Note: escaping fixes in `create-event.ejs`/`edit-event.ejs` were staged surgically (index-blob edit) to exclude the parallel onboarding session's uncommitted alert block from the commit |

## Session Completed (July 5, 2026)

| Feature | Completed | Notes |
|---------|-----------|-------|
| Organizer onboarding simplification design doc | Jul 5 | Docs only, no code changed. Traced the organizer registration workflow end-to-end: signup role choice (`authRoutes.js`) → `/organizer/complete-profile` application (`src/routes/organiser/profile.js`, `OrganiserApplication` model) → manual admin review (`admin/applications.controller.js`, documents purged from R2 on decision) → `organizerStatus` flags, plus the two facts that reframe the whole flow: pending organizers can already create events via the e-signature acknowledgement (`User.canCreateEvents()`, `dashboard.js:402`), and event auto-approval (`event-approval.service.js`) already draws the risk boundary at free+virtual+no-logistics. Documents required today: ID proof mandatory (only mandatory document, ≤5MB, deleted after review); business proof/registration number optional even for company/NGO. Key findings: the document wall is decorative for free events; individuals must invent a "business name"; `under_review` status never set by any code; paid events manually reviewed forever regardless of track record; form/server validation mismatches (address, maxlength). Proposal (4 phases) written to `docs/implementation/organizer-onboarding-simplification.md`: (1) risk-based gating — free virtual events with no application, ID as a "verify to unlock paid events" step, per-type form fields; (2) OCR-assisted auto-approval of `individual` applications reusing the run-proof matched/mismatched/not_detected pattern (server-side OCR required — client-reported results are forgeable for an approval decision), `under_review` as the triage lane; (3) graduated paid-event auto-publish via a versioned v2 `getAutoApprovalEligibility` rule for organizers with N completed events; (4) UX — status page shows current capabilities, stepped checklist, validation fixes. Open questions: N threshold, Tesseract accuracy on PH IDs (needs a spike; fallback is OCR-assisting the admin rather than auto-approving), grandfathering existing pending applications |
| Analysis P2 hardening batch | Jul 5 | Closed every P2 item from the July 5 analysis. **PERF-B** (`rate-limit.middleware.js`): the in-memory fallback `buckets` Map — which is the *only* limiter in production, since Redis turned out to be unconfigured — now stores `windowMs` per bucket, sweeps expired entries lazily on a 60s cadence, and caps at 50,000 entries with oldest-first eviction (Map insertion order), closing both the slow leak and the unique-key memory-exhaustion vector. **PERF-C** (`config/redis.js`): a failed initial `connect()` no longer nulls the client for the process lifetime, and `retryStrategy` retries forever with backoff capped at 30s instead of giving up after 3 attempts; safe because commands fail fast (`maxRetriesPerRequest: 1`) and the limiter already gates on `client.status === 'ready'` — a Redis blip now self-heals. **SEC-E** (`csrf.middleware.js`): the `CSRF_PROTECTION=0` kill-switch is ignored when `NODE_ENV=production` (fail-safe) and logs a loud boot warning in either mode; the integration-test harnesses that set it keep working since they don't run as production. **SEC-D** (same file): session-vs-request CSRF tokens now compared with length-guarded `crypto.timingSafeEqual` instead of `!==`, matching the webhook's approach. **COR-D** (`webhooks/timing-system.js`): HMAC signature comparison length-guards both buffers before `timingSafeEqual`, so a wrong-length signature returns a clean 401 instead of an unhandled `RangeError` 500. **SEC-C** (`upload.service.js`): new `assertDeclaredTypeMatchesBytes` magic-byte check on the verbatim branch of `normalizeFileForUpload` — PDFs must start `%PDF-`, webp must be `RIFF….WEBP`, plus jpeg/png signatures for the conversion-disabled path; sharp-re-encoded images were already covered. Verification: new `tests/p2-hardening.unit.test.js` (5 tests, DB-free — bucket sweep/cap/oldest-eviction/counting via exported `_inMemory` internals, constant-time compare incl. length-mismatch-no-throw, prod-override of the kill-switch via mocked req/res with env restored in `finally`, webhook source guard, PDF/webp accept+reject through the real `_normalizeFileForUpload`); `node --check` clean on all 5 edited files; full unit suite 286/286 |
| Analysis P0/P1 fix batch | Jul 5 | Actioned the top of the July 5 analysis backlog. **SEC-A (P0, stored XSS):** all 12 author-controlled scalar `<%-` outputs in `src/views/admin/blog-review.ejs` (title, excerpt, customCategory, tags, cover/OG URLs, alt text, gallery URLs, contentRaw, seoTitle, seoDescription, moderationNotes) switched to escaped `<%=`; `post.contentHtml` (line 752) deliberately left raw as sanitizer-processed HTML. Also hardened the `<script>` JSON bootstraps on both `blog-review.ejs` and `blog/author-form.ejs` (`JSON.stringify(...).replace(/</g, '<')`) — `post.contentBlocks` is author-controlled and a block containing `</script>` would have broken out of the script tag in the admin session, same class as SEC-A but missed by the analysis. Pulled SEC-B (P2) in early since it's the same file: `contentHtml` hidden-input/textarea and `galleryImageUrlsJson` attribute in `author-form.ejs` now escaped (`<%=` round-trips correctly — browsers decode entities in attribute/textarea values on submit). **COR-A:** `/healthz/sync` registration moved from above the session middleware to just after `attachCsrfToken` in `server.js` — `req.session` now exists, endpoint reachable by admins. **COR-B:** all five async auth guards (`requireAdmin`, `requireFullAdmin`, `requireOrganizer`, `requireApprovedOrganizer`, `requireCanCreateEvents`) wrapped in try/catch → `next(err)`, matching the sibling organizer-event-access/shop-access middleware pattern — a Mongo blip now 500s cleanly instead of hanging 30s. **COR-C:** new `passwordService.getResetTokenTtlMs()` — `Number()`-validated `PASSWORD_RESET_EXPIRY` with a 1-hour fallback (unset/garbage/negative all covered, verified by direct invocation) — used in `authRoutes.js` (the live reset-link write), `email.service.js` (expiry-hours email copy, no more potential "NaN hours"), and `token.service.js`; boot-time warn in `server.js` beside the SESSION_SECRET check (warn not fatal, since the default is safe). Email verification was checked and is *not* affected — its live path hardcodes 24h. **PERF-A:** sparse indexes added on `User.passwordResetToken` and `User.emailVerificationToken` — token redemptions stop being full collection scans; sparse, so only users with outstanding tokens carry entries. Verification (no live DB touched, per no-staging rule): `node --check` clean on all 7 edited JS files; both EJS templates compile; a render test using the *actual* changed template lines confirmed a hostile title (`"><img onerror=…>`) renders inert and a `</script>`-laden content block can't escape the script tag while still JSON-round-tripping; DB-free unit suite 270/275 — the 5 failures predate this session (`audit-source`/`communication-retry-source` tests still grep the pre-DEBT-1 `admin.controller.js` monolith for handlers that now live in `admin/*.controller.js`; flagged in STATUS.md follow-ups). Remaining manual smoke: submit a draft titled `"><b>x` and eyeball `/admin` blog review |
| Full codebase analysis (fresh eyes) | Jul 5 | Read-only analysis of all of `src/` (~73k LoC: 71 services, 19 controllers, 25 routes, 34 models, 8 middleware, 131 EJS views, 98 tests) across six dimensions — security, correctness, performance, code quality/refactoring, dependencies, test suite. Deliverables: `docs/analysis/2026-07-05/{README,security,correctness,code-quality,performance,test-suite,dependencies}.md`, each with severity-rated findings citing verified `file:line` and a "handled correctly ✓" section. **Headline finding (P0):** stored XSS in `src/views/admin/blog-review.ejs` — author-controlled scalar fields (`title`, `excerpt`, tags, URLs, SEO fields, `contentRaw`) are emitted with unescaped `<%-` into attributes/textareas and execute in the admin reviewer's session; fix is to switch them to `<%=` (neighbouring `<%= category %>` already does this correctly). **P1:** `PASSWORD_RESET_EXPIRY` unset → `parseInt(undefined)` NaN → every reset link silently fails; `/healthz/sync` registered before session middleware so it always 403s (dead); async auth guards (`requireAdmin`/`requireFullAdmin`/…) lack try/catch → a Mongo blip hangs the request 30s; `passwordResetToken`/`emailVerificationToken` unindexed → collection scans. **P2:** in-memory rate-limit fallback Map never pruned (leak/DoS); Redis permanently disabled after one transient startup failure; `CSRF_PROTECTION=0` kill-switch should fail-safe in prod; timing-webhook `timingSafeEqual` throws 500 on wrong-length signature. **P3 debt:** split `page.controller.js` (3.5k), `blog.controller.js` (2.6k), `submission.service.js` (2.1k); delete dead `src/config/db.js` and unmounted `src/routes/event.routes.js` (byte-identical to `certificateVerification.routes.js`); centralise `process.env` reads into a validated boot-time config. **Verified good:** June 24 SEC-1/2/3 fixes all hold; admin authz (requireAdmin + requireFullAdmin tiering) thorough and rate-limited; no SQL injection (parameterised throughout, allowlisted identifiers in purge services); upload pipeline re-encodes images via sharp; `npm audit` clean (0 vulns); no unused deps (`feed` is used via dynamic `import()`). Overall grades: Security A−, Correctness B+, Performance B+, Code quality B+, Dependencies A−, Test suite B. Docs only, no code changed |
| Event promotion "queued emails never send" fix | Jul 5 | Diagnosed why admin promotion campaigns reported most recipients as `queued` with the mails never arriving (Jul 5 campaign: 4 sent / 42 queued at 4/100 quota). Two root causes: (1) `dispatchEventPromotionCampaign` fired all recipients concurrently via `Promise.allSettled`, tripping Resend's ~2 req/s limit — everything past the first burst 429'd into the `CommunicationRetry` queue; (2) `buildCommunicationRetryKey` omitted `campaignId`, so a re-send campaign to the same recipient+event collided with the previous campaign's finished (`sent`/`dead`) retry job — `$setOnInsert` never reset it, and the email was silently swallowed (exactly what happened to the Jul 5 campaign colliding with Jul 3's). Fixes: `campaignId` added to the retry idempotency key (`reliable-communication.service.js`); dispatch now sends sequentially with a configurable gap (`EVENT_PROMOTION_SEND_INTERVAL_MS`, default 600ms); since a 500-recipient campaign now takes minutes, both the admin controller and organiser route dispatch via new `dispatchEventPromotionCampaignInBackground()` (finalises campaign counts/status on completion) and redirect immediately with a "Promotion started" message — progress visible in the existing Recent Campaigns table via the `sending` status; `recipientCount`/`selectedCount` now written at campaign creation so `getQuotaUsed()` counts in-flight campaigns (closes a quota race the background dispatch would otherwise open). Analysis + deployment notes in `docs/features/event_promotion_queued_email_fix.md`; swallowed Jul 5 recipients are recovered by simply re-sending the campaign after deploy. New `tests/communication-retry-key.unit.test.js` (3 tests, DB-free) + updated `tests/event-promotion-source.unit.test.js` |
...
Partially in `docs/implementation/user-management-improvement-draft.md` — review for existing implementation.
...
The cross-role workflow improvement program in `docs/archive/reviews/2026-07-workflow-analysis/README.md` completed all seven implementation waves on July 15, 2026. Remaining work is production/device validation and metric collection; it is tracked in `docs/STATUS.md`, not as an unfinished product priority.

The runner desktop dashboard task-first layout follow-up is also implemented. Its remaining checkpoint is visual confirmation with production-shaped authenticated data at the documented desktop widths.
