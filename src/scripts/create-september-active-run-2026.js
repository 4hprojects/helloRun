...
} = require('../content/events/september-active-run-2026');

const APPLY = process.argv.includes('--apply');
const ASSET_DIR = path.resolve(__dirname, '../../assets/events/september-active-run-2026');
const BANNER_PATH = path.join(ASSET_DIR, 'september-active-run-2026-banner.png');
const POSTER_PATH = path.join(ASSET_DIR, 'september-active-run-2026-poster.png');
const LOGO_PATH = path.join(ASSET_DIR, 'september-active-run-2026-badge.png');
...
async function main() {
  ...
  const previewPayload = buildSeptemberActiveRunEventPayload({
    organizerId: admin._id,
    approvedBy: admin._id,
    referenceCode: 'DRY-RUN',
    bannerUrl: 'https://cdn.example.invalid/september-active-run-2026-banner.webp',
    logoUrl: 'https://cdn.example.invalid/hellorun-logo.webp',
    badgeImageUrl: 'https://cdn.example.invalid/september-active-run-2026-badge.webp',
    posterUrl: 'https://cdn.example.invalid/september-active-run-2026-poster.webp'
  });
  ...
  try {
    ...
    event = new Event(buildSeptemberActiveRunEventPayload({
      organizerId: admin._id,
      approvedBy: admin._id,
      referenceCode,
    ...
    template.content.bodyText = 'This certifies that {{runnerName}} completed the {{distance}} category of {{eventTitle}} through consistent, verified movement.';
    template.styleOptions.primaryColor = '#086534';
    template.styleOptions.accentColor = '#F05A0A';
    template.styleOptions.secondaryAccentColor = '#F5C45E';
    await template.save();
    ...
    try {
      ...
      const connectivityError = /ENOTFOUND|ENETUNREACH|CONNECT_TIMEOUT|ECONNREFUSED|ETIMEDOUT/i.test(String(error?.message || error));
